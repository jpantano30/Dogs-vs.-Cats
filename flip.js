document.addEventListener('DOMContentLoaded', function () {
  ...
  function flipCoin() {
    ...
    //elapsed time during the coin flip animation.
    let currentTime = 0
    //total duration of the coin flip animation
    ...
    const flipInterval = setInterval(() => {
      ...
      // increment currentTime by 100ms each interval - time passing
      currentTime += 100 
      ...
      if (currentTime >= 1000) {
        ...
        // sets the result to the random result
  ...
  // resets the coin toss and game state
  resetCoinBox()

  // clears game board and resets game state
  init()
  ...
  function checkWinner (){
    console.log('winner:', winner)
    // iterates through wining combos
    for (let i = 0; i < winningCombos.length; i++){
      // Calculates the sum of values at the three positions in the array from the current winning combination
        sum = Math.abs(
          board[winningCombos[i][0]] + 
          board[winningCombos[i][1]] + 
          board[winningCombos[i][2]])
        //Checks if the sum is equal to 3 for win
        if (sum === 3) {
          // If the first position in the winning combination is 1, increments player 1 score.
            ...
            //Returns the value at the first position in the winning combination (1 or -1)
            return board[winningCombos[i][0]]
        }
    }
    // If the board is full and no winning combination has been found, returns a tie
    if (!board.includes(null)) {
        tie += 1
        return 'Tie'
    }
    // If the board is not full and no winning combination has been found, returns null
    return null
  }
